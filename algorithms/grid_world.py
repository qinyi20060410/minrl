...
class GridWorld:
    ...
    def det_transition_matrix(self):
        ...
        )  # the latent dimension is 'num_states-1', as one movement actually can jump to any state except itself
        rtm = np.zeros(
            (self.num_states, len(self.action_space))
        )  # the latent dimension is 3, as there are three possible reward in the grid example
