...
def write_excel(hyperparameters, img_path, bias, xlsx_path, highlights=[], gap=1):
    """
    ...
    for key, value in hyperparameters.items():
        ...
        if key in highlights:
            cell.fill = PatternFill(
                start_color="F5C142", end_color="F5C142", fill_type="solid"
            )  # 高亮黄色
        row_num += 1
    ...
    ws.merge_cells(
        f"{column_letter}{row_bias + 1}:{column_letter}{row_bias + hyperparameters_cnt}"
    )
