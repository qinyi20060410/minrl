...
def plot_policy(
    ...
    ax_pre_frame_probabilities.set_title(
        "pre frame", fontproperties="SimHei", fontsize=10
    )
    ax_trajectory.set_title("trajectory", fontproperties="SimHei", fontsize=10)
    ax_now_frame_probabilities.set_title(
        "now frame", fontproperties="SimHei", fontsize=10
    )
    ...
    for i in range(rows):
        for j in range(columns):
            ...
            if mp[i][j] == "T":
                color = "#65FFFF"
            ax_pre_frame_probabilities.add_patch(
                plt.Rectangle(
                    (j, rows - 1 - i),
                    1,
                    1,
        ...
        for j in range(columns):
            for k, action in enumerate(actions):
                ...
                if action == "stay":
                    ...
                    ax_pre_frame_probabilities.text(
                        j + 0.5 + ox,
                        rows - 0.5 - i + oy,
                        f"{prob * 100:.0f}",
                        color=color,
                ...
                arrow = FancyArrowPatch(
                    (j + 0.5 + ox_a, rows - 1 - i + 0.5 + oy_a),
                    (j + 0.5 + dx * len_scale, rows - 1 - i + 0.5 + dy * len_scale),
                ...
                ax_pre_frame_probabilities.text(
                    j + 0.5 + ox_a + dx * 0.3 + ox_t,
                    rows - 0.5 - i + oy_a + dy * 0.3 + oy_t,
                    f"{prob * 100:.0f}",
                    color=color,
                    ha="center",
                    va="center",
    ...
    for i in range(rows):
        for j in range(columns):
            ...
            if mp[i][j] == "T":
                color = "#65FFFF"
            ax_now_frame_probabilities.add_patch(
                plt.Rectangle(
                    (j, rows - 1 - i),
                    1,
                    1,
        ...
        for j in range(columns):
            for k, action in enumerate(actions):
                ...
                if action == "stay":
                    ...
                    ax_now_frame_probabilities.text(
                        j + 0.5 + ox,
                        rows - 0.5 - i + oy,
                        f"{prob * 100:.0f}",
                        color=color,
                ...
                arrow = FancyArrowPatch(
                    (j + 0.5 + ox_a, rows - 0.5 - i + oy_a),
                    (j + 0.5 + dx * len_scale, rows - 0.5 - i + dy * len_scale),
                    arrowstyle=arrowstyle,
                    mutation_scale=5,
                    lw=arrow_width,
                    color=color,
